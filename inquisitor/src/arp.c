...
void fill_field_packet(t_inquisitor *inquisitor, t_packet *packet, const int8_t *ip_src, const int8_t *ip_target, const int8_t *mac_target) {
  uint32_t int_addr;
...
void init_dest_sock(struct sockaddr_ll *dest, t_inquisitor *inquisitor) {
...
void send_fake_arp_packet(t_inquisitor *inquisitor, uint32_t packet_nbr) {
  ...
  switch (packet_nbr) {
    ...
    case 2:
      ...
      break;
    default:
      dprintf(2, "Error packet_nbr %s", __func__ );
      if (g_sock)
        close(g_sock);
      exit(EXIT_FAILURE);
  }
